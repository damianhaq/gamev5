...
export class Character {
  ...
  constructor(public hp: number, public id: string, public attackSpeed: number = 0) {
    this.x;
    this.y;
    this.radius;
    this.id = id;
  ...
  update(deltaTime: number, index: number | null = null) {
    this.draw(deltaTime);
  ...
  drawHpBar() {
    if (this.hp < this.maxHp) {
      c.drawImage(
        ...
        this.hpBarSprite.w,
        this.hpBarSprite.h,
        this.x + dimensions.map.x - this.hpBarSprite.w,
        this.y + dimensions.map.y - this.hpBarSprite.h + this.radius + 12,
        this.hpBarSprite.w * 2,
        this.hpBarSprite.h * 2
      );

      const currentWidth = calculatePercentage(
        this.hp,
        this.maxHp,
        this.hpBarSpriteFull.w
      );
      c.drawImage(
        spriteSheet,
        this.hpBarSpriteFull.x,
        this.hpBarSpriteFull.y,
        currentWidth,
        this.hpBarSpriteFull.h,
        this.x + dimensions.map.x - this.hpBarSpriteFull.w,
        this.y + dimensions.map.y - this.hpBarSpriteFull.h + this.radius + 12,
        currentWidth * 2,
        this.hpBarSpriteFull.h * 2
      );
    }
  }
  ...
  draw(deltaTime: number) {
    ...
    if (this.blink > 0) this.blink -= deltaTime;
    ...
    if (this.blink > 0) c.filter = "brightness(2)";
    ...
    c.drawImage(
      ...
      fixedPosX, // Przesunięcie wzdłuż osi X
      this.y +
        dimensions.map.y -
        this.spriteSheetData[this.animData.animState].h -
        this.animData.addY,
  ...
  // in update
  makeKnockback(deltaTime: number) {
    if (this.knockback.duration > 0 && !game.isPause) {
      ...
      if (this.knockback.duration < 0) this.knockback.duration = 0;
    } else {
  ...
  getDamage(value: number, color: string, id: string | null = null) {
    ...
    instances.appearingText.push(
      new AppearingText(
        ...
        this.y,
        500,
        this.id === "playerCh" ? dmgAfterArmor.toString() : value.toString(),
        16,
        color
      )
    );
