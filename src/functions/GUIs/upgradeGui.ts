...
export function upgradeGui(gui: GUI) {
  const d = {
    x: 400,
    y: 150,
    w: 730,
...
function item(d, gui: GUI, i: number, id: string) {
  // console.log(id);

  gui.clickFrame(
    d.x + d.padding,
    d.y + d.item.h * i + d.item.margin * i + d.padding,
    d.w - d.padding * 2,
    d.item.h,
    testf
  );
  ...
  gui.text(
    d.x + d.padding + 10,
    d.y + d.item.h * i + d.item.margin * i + d.padding + 30,
    id,
    12,
    game.font.main,
    game.colors.blue,
...
function randomSkills() {
  ...
  for (let i = 0; i < stats.skills.upgrade.amount; i++) {
    stats.skills.upgrade.skillsRandomPicked.push(
      skillList[randomNumber(0, skillList.length - 1)]
    );
  }
